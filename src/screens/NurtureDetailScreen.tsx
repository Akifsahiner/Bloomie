...
export default function NurtureDetailScreen() {
  ...
  const allNurtureLogs = recentLogs.filter(log => log.nurture_id === nurtureId);

  // Filter logs based on search and filters
  const filteredLogs = useMemo(() => {
    let logs = [...allNurtureLogs];

    // Search filter
    if (searchQuery.trim()) {
      const query = searchQuery.toLowerCase();
      logs = logs.filter(log => 
        log.raw_input?.toLowerCase().includes(query) ||
        log.parsed_notes?.toLowerCase().includes(query) ||
        log.parsed_action?.toLowerCase().includes(query)
      );
    }
  ...
  }, [allNurtureLogs, searchQuery, filters]);

  const nurtureLogs = filteredLogs;
      ...
      >
        ...
        <View style={styles.statsContainer}>
          <TouchableOpacity
            ...
            onPress={() => (navigation as any).navigate('Statistics', { nurtureId: nurtureId })}
            activeOpacity={0.8}
          >
            <Text style={[styles.statValue, { color: colors.main }]}>{allNurtureLogs.length}</Text>
          ...
          <TouchableOpacity
            style={styles.statCard}
            onPress={() => (navigation as any).navigate('PhotoGallery', { nurtureId: nurtureId })}
            activeOpacity={0.8}
          >
            <Text style={[styles.statValue, { color: colors.main }]}>
              {allNurtureLogs.filter(log => log.photo_urls && log.photo_urls.length > 0).length}
            </Text>
        ...
        <View style={styles.section}>
          ...
          {nurtureLogs.length > 0 ? (
            <View style={styles.timelineContainer}>
              {nurtureLogs.map((log) => (
                <View key={log.id} style={styles.timelineEntry}>
                  <View style={[styles.timelineDot, { backgroundColor: colors.main }]} />
                  <View style={styles.timelineContent}>
                    ...
                    {log.photo_urls && log.photo_urls.length > 0 && (
                      <View style={styles.timelinePhotos}>
                        {log.photo_urls.slice(0, 3).map((photo, idx) => (
                          <Image
                            key={idx}
                            source={{ uri: photo }}
            ...
            <View style={styles.emptyActivity}>
              <MaterialCommunityIcons name="magnify" size={48} color={Colors.gray[300]} />
              <Text style={styles.emptyActivityText}>No entries found</Text>
        ...
        <View style={styles.section}>
          ...
          {smartInsights.length > 0 ? (
            ...
            <View style={styles.insightsCarousel}>
              ...
              {allNurtureLogs.slice(0, 3).map((log, index) => {
      ...
      <FilterModal
        ...
        onApply={(newFilters) => {
          setFilters({
            ...
            nurtureTypes: newFilters.nurtureTypes as any[],
            nurtureIds: newFilters.nurtureIds,
          });
        }}
...
const styles = StyleSheet.create({
  ...
  emptyActivitySubtext: { fontSize: 14, color: Colors.gray[400], marginTop: 8, textAlign: 'center' },
  // Modal Styles
